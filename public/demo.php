...
$bootstrap = $profiler->start('Bootstrap', array('lorem' => 'ipsum'), 'Application');
usleep($wait(50));
$profiler->stop($bootstrap);

$logger->info('Bootstrap has finished', array('some' => 'context'));

$sessionLoad = $profiler->start('Session::load', array('lorem' => ''), 'Sessions');
usleep($wait(45));
$profiler->stop($sessionLoad);

$dispatcher = $profiler->start('Dispatcher', array('abc' => '123', 'foobar' => true), 'Dispatcher');
usleep($wait(25));

    $logger->warning('Not everything ready to go', array('some' => 'context'));

    $router = $profiler->start('Router', array('some' => 'value', 'foobar' => 123), 'Dispatcher');
    usleep($wait(150));

    $profiler->stop($router);

    $logger->alert('Route not found', array('lorem' => 'ipsum', 'foobar' => true));

    $controller = $profiler->start('Controller', array('some' => 'value', 'foobar' => 123, 'array' => array('foo' => 'bar', 'lorem' => true, 'ipsum' => 1.5)), 'Application');
    usleep($wait(200));

        $view = $profiler->start('PDO::exec', array('query' => 'DELETE FROM users WHERE email = "foo@bar.com"'), 'Database');
        usleep($wait(50));
        $profiler->stop($view);

        $view = $profiler->start('PDO::query', array('query' => 'SELECT userLoginEmail, userID, userLoginPassword FROM users LIMIT 2500;'), 'Database');
        usleep($wait(350));
        $profiler->stop($view);

        $view = $profiler->start('View::render', array('data' => array('user' => array('name' => 'John Doe', 'age' => 26)), 'foobar' => 123), 'View');
        usleep($wait(10));
        $profiler->stop($view);

        $logger->notice('Undefined variable: $foobar', array('some' => 'context'));

        $database = $profiler->start('\Fabfuel\Mongo\Collection\Foobar::find', array('query' => array('user' => '54815c5081de416a770041a7', 'active' => true), 'foobar' => 123), 'MongoDB');
        usleep($wait(25));
        $profiler->stop($database);

        $logger->debug('Analyze query', array('query' => array('user' => 12312), 'foobar' => 123));

        $view = $profiler->start('View::render', array('data' => array('user' => array('name' => 'John Doe', 'age' => 26)), 'foobar' => 123), 'View');
        usleep($wait(10));
        $profiler->stop($view);

        $logger->critical('Lorem Ipsum', array('some' => 'context'));
    ...
    $logger->error('Foobar not found', array('some' => 'context'));

$profiler->stop($dispatcher);

$sessionWrite = $profiler->start('Session::write', array('lorem' => 'ipsum'), 'Sessions');
usleep($wait(45));
